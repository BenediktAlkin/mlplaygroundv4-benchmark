...
def calculate_effective_batch_size_per_device(effective_batch_size, world_size=None):
    ...
    assert effective_batch_size % world_size == 0, "effective_batch_size needs to be multiple of world_size"
        ...
        if isinstance(sample, (list, tuple)):
            ...
            for item in sample:
                if isinstance(item, (list, tuple)):
                    data.append([einops.repeat(entry, "one ... -> (bs one) ...", bs=batch_size) for entry in item])
                else:
                    data.append(einops.repeat(item, "one ... -> (bs one) ...", bs=batch_size))
        else:
            data = einops.repeat(sample, "one ... -> (bs one) ...", bs=batch_size)
        # wrap into tuple
        if isinstance(data, list):
            data = tuple(data)
        # scale batch_size of ctx
        ctx = {
            k: einops.repeat(v, "one ... -> (bs one) ...", bs=batch_size) if torch.is_tensor(v) else v
