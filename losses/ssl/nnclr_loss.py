...
class NnclrLoss(nn.Module):
    def __init__(self, temperature=0.2, non_negativity=None):
        super().__init__()
    ...
    def forward(self, projected_preswap, projected_postswap, predicted, reduction="mean"):
        ...
        # non-negativity
        if self.non_negativity is None:
            pass
        elif self.non_negativity == "relu":
            projected_preswap = F.relu(projected_preswap)
            projected_postswap = F.relu(projected_postswap)
            predicted = F.relu(predicted)
        else:
            raise NotImplementedError(f"invalid non_negativity: {self.non_negativity}")
