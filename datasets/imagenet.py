...
class ImageNet(ImageFolder):
    ...
    def __init__(
        ...
        if version in ["imagenet_a", "imagenet_r"]:
            ...
            assert distortion in ImageNet.IMAGENET_C_DISTORTIONS, distortion
        ...
        if semi_version is not None:
            with open(f"resources/semi/{Path(semi_version).with_suffix('.yaml')}") as f:
                self.semi_labeled_uris = set(yaml.safe_load(f))
        else:
