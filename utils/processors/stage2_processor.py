    ...
    def __call__(self, root):
        ...
        for head_name, head in heads.items():
            ...
            for i in range(len(split)):
                # loss kind + head kind
                if split[i] == "massl":
                    loss_functions[head_name]["kind"] = "massl_loss"
                    head["kind"] = "ssl.massl_head"
                elif split[i] == "nnclr":
                ...
                elif split[i] == "nnclrrelu":
                    loss_functions[head_name]["kind"] = "nnclr_loss"
                    loss_functions[head_name]["non_negativity"] = "relu"
                    head["kind"] = "ssl.nnclr_head"
                ...
                # loss partitions
                elif split[i].startswith("part"):
                    num_partitions = int(split[i][len("part"):])
                    loss_functions[head_name]["num_partitions"] = num_partitions
                # loss teacher temperature (e.g. dino)
                elif split[i].startswith("ttemp"):
                    if "to" in split[i]:
                        start, end = split[i][len("ttemp"):].split("to")
                        ttemp_start = self.__from_float_str(start)
                        ttemp_end = self.__from_float_str(end)
                        loss_functions[head_name]["teacher_temperature"] = ttemp_end
                        loss_functions[head_name]["teacher_temperature_schedule"] = [
                            dict(
                                schedule=dict(
                                    kind="linear_increasing_schedule",
                                    start_value=ttemp_start,
                                ),
                                end_epoch=4,
                            ),
                        ]
                ...
                # head pooling (cls)
                elif split[i].startswith("cls") or split[i].startswith("crossattn"):
                    if split[i] == "cls":
                        head["pooling"] = dict(kind="class_token")
                    elif split[i] == "clsavg":
                        head["pooling"] = dict(kind="concat_class_average")
                    elif split[i] == "crossattn":
                        head["pooling"] = dict(kind="cross_attention_pooling", num_query_tokens=0)
                    elif split[i] == "crossattnconcat":
                        head["pooling"] = dict(kind="cross_attention_pooling", num_query_tokens=0, concat_query=True)
                    else:
                        if split[i].endswith("attn"):
                            ...
                            block_idx = int("".join(c for c in split[i] if c.isdigit()))
                            pooling_kind = "".join(c for c in split[i] if not c.isdigit())
                            if pooling_kind == "cls":
                                pooling = dict(kind="class_token")
                            elif pooling_kind == "clsavg":
                                pooling = dict(kind="concat_class_average")
                            elif pooling_kind == "crossattn":
                                pooling = dict(kind="cross_attention_pooling", num_query_tokens=0)
                            elif pooling_kind == "crossattnconcat":
                                pooling = dict(kind="cross_attention_pooling", num_query_tokens=0, concat_query=True)
                            else:
