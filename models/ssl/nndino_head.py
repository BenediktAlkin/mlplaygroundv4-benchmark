...
class NndinoHead(SingleModelBase):
    def __init__(
            self,
            proj_hidden_dim,
            queue_size,
        ...
        # queue
        self.queue = NnclrQueue(size=queue_size, dim=bottleneck_dim, **(queue_kwargs or {}))
        ...
        assert type(self).after_update_step == NndinoHead.after_update_step
    ...
    @staticmethod
    def _forward_to_bottleneck(projector, x):
        for layer in projector[:-1]:
            x = layer(x)
        return x
    ...
    def forward(
        ...
        # forward student
        bottleneck = self._forward_to_bottleneck(projector=self.projector, x=x)
        predicted = self.projector[-1](bottleneck)
        # forward teacher
        with torch.no_grad():
            if self.momentum_projector is not None:
                bottleneck = self._forward_to_bottleneck(projector=self.momentum_projector, x=momentum_x)
                bottleneck, metrics = self.queue(x=bottleneck, idx=idx, cls=cls, confidence=confidence)
                projected = self.momentum_projector[-1](bottleneck)
                if self.training:
                ...
                if batch_size is not None and num_teacher_views is not None:
                    bottleneck = bottleneck.detach()[:batch_size * num_teacher_views]
                    bottleneck, metrics = self.queue(x=bottleneck, idx=idx, cls=cls, confidence=confidence)
                    projected = self.projector[-1](bottleneck)
                    if self.training:
                        self.center_buffer.append(torch.mean(projected, dim=0))
                else:
    ...
    def after_update_step(self):
        # TODO queue is not independent of gradient accumulation steps, but it should be negligible
        # update center
