...
class NnclrNoswapHead(SingleModelBase):
    def __init__(
            self,
            output_dim,
            pooling,
        ...
        assert type(self).after_update_step == NnclrNoswapHead.after_update_step
    ...
    def forward(
        ...
        if self.momentum_projector is not None:
            ...
            if batch_size is not None and num_teacher_views is not None:
                projected = projected[:batch_size * num_teacher_views]
