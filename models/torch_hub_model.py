...
class TorchHubModel(SingleModelBase):
    def __init__(self, repo, model, mode=None, drop_path_rate=None, pretrained=True, **kwargs):
        ...
        self.model = torch.hub.load(repo, model, pretrained=pretrained, **kwargs)
        # populate static_ctx
        ...
        if model.startswith("aim"):
            self.static_ctx["num_aux_tokens"] = 0
        else:
            if mode is None:
                self.head = None
    ...
    def forward(self, x):
        if type(self.model).__name__ == "AIM":
            x = self.model(x)
        else:
